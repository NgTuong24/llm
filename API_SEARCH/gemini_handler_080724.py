...
class GeminiHandler:
    def __init__(self):
        # self.model_llm = VertexAI(model_name="gemini-1.0-pro-002", temperature=1)
        self.model_llm = GoogleGenerativeAI(model="models/gemini-1.5-pro-001", temperature=1)
        ...
        # self.search_api.max_results = 2
        google_api_key = "AIzaSyBJWAfYlrLeVeJhkHJDCqDnuN3SDCNZqbM"
        search_engine_id = "6709e10c035e54a0c"
        wrapper = GoogleSearchAPIWrapper(google_cse_id=search_engine_id,
                                         google_api_key=google_api_key)
        self.search_api = GoogleSearchResults(api_wrapper=wrapper)
        self.search_api.num_results = 5
        self.create_chain()
    ...
    def run_analytic(self):
        template_analytic = """
            回答は日本語でなければなりません。
            1.
            GDP成長、購買力、業界平均成長、市場トレンド、市場の成長ドライバーなどはどのデータで収益予測したのかを提示してください。
            ...
            回答は日本語でなければなりません。  
        """
        return self.model_llm.invoke(template_analytic)

    def run_get_name_entity(self, output_all):
        """
        :param output_all: output of PL + output of analytic
        :return:  [name_entity]
        """
        return self.chain_get_name_entity.invoke(output_all)

    def final_output(self, list_entity):
        """
        :param list_entity: [name_entity, ..]
        :return:    summary
                    Source
                      title
                      link
        ...
        if len(list_entity) != 0:
            for ind, entity in enumerate(list_entity):
                ...
                if len(classify[0]) != 0:
                    summary_text = self.chain_summary.invoke({"snippets": classify[0]})
                    final_result += f"\n\n//***{list_entity[ind]}***//" + '\n'
    ...
    def create_chain_get_name_entity(self):
        template_get_name_entity = """
            ...
            {context}はコンテキストです。
            {context}からキーワードを、上記の予測の根拠を説明するために使用したデータ (存在する場合) を含む内容のフレーズで絶対に抽出してください。
            結果は以下のJSON型式で出してください。返される結果は、キーが 1 つだけの JSON 構造です。
            "keywords": [GDP成長??, 購買力??, 業界平均成長??, 市場トレンド??, 市場の成長ドライバー??, など];
            注記：
        ...
        # template_get_name_entity = """
        #     あなたは 生成AI アシスタントモデルで、キーワードを抽出しています。
        #     キーワードを、上記の予測の根拠を説明するために使用したデータ (存在する場合) を含む内容のフレーズで絶対に抽出してください。
        #     結果は以下のJSON型式で出してください。返される結果は、キーが 1 つだけの JSON 構造です。
        #     "keywords": [GDP成長??, 購買力??, 業界平均成長??, 市場トレンド??, 市場の成長ドライバー??, など];
        #     注記：
        #     1. 見出しを最も一般的なレベルに並べ替え、数を 10 未満に制限します。
    ...
    def create_summary_snippet(self):
        template_summary_search = """
            文脈に基づいて
            検索した後に GoogleAPISearch で見つかったスニペット {snippets}
            上記出してくれた収益予測の根拠説明を裏付けるために業界の動向、政治的および経済的などの要因に関するデータを収集しました。結果は、検索された政府、監査会社、調査会社などの最も信頼できる情報源からの市場、業界、経済、政治レポートで説明して正当化してください。
            注:
            1. 回答は日本語でなければなりません。
    ...
    def format_output_of_search(self, docs: str):
        ...
        for ind, doc in enumerate(list_):
            ...
            # LLM check title
            snippet = doc[ind_2 + 10:]
            title = doc[ind_0 + 7: ind_1 - 3]
            links = doc[ind_1 + 7: ind_2 - 3]
            check = self.chain_check_title.invoke(title).strip().lower()
            if "yes" in check or "はい" in check:
                document_snippets += snippet
                document_title += title
                if ind != 0:
                    document_source += "\n"
                ...
                # print(f"ADD: {links}")
            elif "no" in check or "いいえ" in check:
                print(f"REJECTED: {links}")
                pass

            else:
                print("CHECK LLM check title answer: ", check)
        return document_snippets, document_title, document_source

    # def run_get_name_entity(self):
    #     template_get_name_entity = """
    #         あなたは 生成AI アシスタントモデルで、キーワードを抽出しています。
    #         キーワードを、上記の予測の根拠を説明するために使用したデータ (存在する場合) を含む内容のフレーズで絶対に抽出してください。
    #         結果は以下のJSON型式で出してください。返される結果は、キーが 1 つだけの JSON 構造です。
    ...
    #         2. 日本語で回答する必要があります。
    #     """
    #     result_text = self.model_llm.invoke(template_get_name_entity)

    # def create_chain_analytic(self):
    #     # template_analytic = """
    #     #     回答は日本語でなければなりません。
    #     #     1. 以下の質問の回答は{context}になります。
    #     #     GDP成長、購買力、業界平均成長、市場トレンド、市場の成長ドライバーなどはどのデータで収益予測したのかを提示してください。
    #     #     以前に提供した情報と重複する情報は絶対に出さないでください。
    #     #     2. 以下の質問の回答は{context}になります。
    #     #     収益を予測するためにどのような方法を使用していますか？上記の予測を簡単に提示できるように教えてください。
    #     #     以前に提供した情報と重複する情報は絶対に出さないでください。
    #     #     回答は日本語でなければなりません。
    #     # """
